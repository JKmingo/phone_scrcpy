...
public class Adb {
    ...
    public Adb(Context context, String address, int port) throws Exception {
    ...
    public Adb(Context context) throws Exception {
        this.context = context;
        if (keyPair == null) {
            keyPair = readAdbKeyPair();
        }
        channel = new UsbChannel(context);
        connect(keyPair);
    }

    public Adb(Context context, UsbDevice usbDevice) throws Exception {
        this.context = context;
        if (keyPair == null) {
            keyPair = readAdbKeyPair();
        }
        channel = new UsbChannel(context, usbDevice);
        connect(keyPair);
    }
    ...
    // 生成密钥
    public AdbKeyPair reGenerateAdbKeyPair() {
        try {
            Pair<File, File> adbKeyFile = getAdbKeyFile(context);
    ...
    // 读取密钥
    public AdbKeyPair readAdbKeyPair() {
        try {
